...
impl Block {
    pub(crate) fn parse(parser: &mut Parser) -> SyntaxResult<Self> {
        ...
        Ok(Self {
            span: open_brace.span.union(close_brace.span),
            open_brace,
            statements,
            close_brace,
        })
...
mod test {
    ...
    test_parse!(block_single: "{ let x = 5 }" => Block::parse => Block { statements: [Statement::Let(..)], .. });
    test_parse!(block_single_semi: "{ let x = 5; }" => Block::parse => Block { statements: [Statement::Let(..)], .. });
    test_parse!(block_single_lines: "{
        let x = 5;
    }" => Block::parse => Block { statements: [Statement::Let(..)], .. });
    test_parse!(block_semis: "{ let x = 5; return x * 2; }" => Block::parse => Block { statements: [Statement::Let(..), Statement::Expression(..)], .. });
    test_parse!(block_end_no_semi: "{ let x = 5; return x * 2 }" => Block::parse => Block { statements: [Statement::Let(..), Statement::Expression(..)], .. });
    test_parse!(block_lines: "{
        let x = 5
        return x * 2
    }" => Block::parse => Block { statements: [Statement::Let(..), Statement::Expression(..)], .. });
