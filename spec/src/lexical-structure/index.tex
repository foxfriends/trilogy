...
A number suffixed with \texttt{i} is treated as an imaginary number, for
example \val{13 + 5i} can be used to represent the complex number  $13 + 5i$.
The number may be any valid real number (including rational or floating
point numbers). A negative complex number $15 - 5i$ is written as usual
(\val{15 - 5i}).

\subsection{String and Character Literals}

Strings in \Trilogy{} are always enclosed in double quotes (\texttt{"}).
They may contain interpolations, so the tokens and syntax for strings is
notably more complex than just strings. Special ``tagged templates'' a
\texttt{\$} and an identifier.
