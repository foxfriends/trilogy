...
An execution ending is typically not treated as an error; such endings are often expected
to cull unnecessary branches of execution. The only time an execution ending is treated as
an error is when it is the last execution in the program, in which case it is a
\emph{last execution ended error}.
