...
There are two forms to a \kw{let} statement: the common unification
case, and the more general \Law{} rule application.
...
\begin{prooftree}
    \AxiomC{$\Gamma \vdash x = y$}
    \AxiomC{$\Gamma, \{ a_i\ |\ a_i := x \} :> \Phi$}
    \LeftLabel{\label{sem:declaration}Declaration}
    \BinaryInfC{$\Gamma \vdash \kw{let}\ x = y \Rightarrow \Phi$}
\end{prooftree}

What is more unique to \Trilogy{} is the branching and fizzling capability of the
\kw{let} statement, which may lead to multiple executions of the program running
in parallel.

\begin{prooftree}
    \AxiomC{$\Gamma \vdash x \neq y$}
    \LeftLabel{\label{sem:declaration}Declaration\textsuperscript{$\bot$}}
    \UnaryInfC{$\Gamma \vdash \kw{let}\ x = y : \bot$}
\end{prooftree}
