...
If control runs off the end of the procedure's body block, the call evaluates to the final
value of the block. A procedure may cause an execution to end or branch, in which case all
evaluations are propgated to the call site, conceptually causing the call to end or branch
respectively.
...
\begin{prooftree}
    ...
    \def\extraVskip{3.5pt}
    \BinaryInfC{$\Gamma,\setof{a_i\given a_i\defby P_i} \vdash M \Rightarrow \Phi$}
    \LeftLabel{Procedure\textsubscript{2}}
    \def\extraVskip{2pt}
    \BinaryInfC{$\Gamma \vdash p : \texttt{!(}\tau_{1\hdots n}\texttt{)} \rightarrow \Phi$}
\end{prooftree}
...
\begin{prooftree}
    ...
    \AxiomC{$x_i = P_i$}
    \def\extraVskip{3.5pt}
    \BinaryInfC{$\Gamma,\setof{a_i\given a_i\defby P_i} \vdash M: \bot$}
