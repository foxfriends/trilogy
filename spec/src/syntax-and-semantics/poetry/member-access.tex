...
The member access operator (\op{.}) deserves its own section, for its relatively
complex behaviour. Some values contain other values within them, and those values
are accessed using the member access syntax. Though this uses a \op{.}, as seen in
many other languages, in \Trilogy{} that \op{.} truly is just an operator: the value
on the right side is interpreted as an expression like any other.
