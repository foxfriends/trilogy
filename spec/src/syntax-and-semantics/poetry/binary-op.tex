...
The regular bitwise operators extend the shorter operand with 0s on the right
until both are of the same length.

The shift operators come in three flavours: standard (\op{\textasciitilde>}
and \op{<\textasciitilde}), extending (\op{\textasciitilde\textasciitilde>}
and \op{<\textasciitilde\textasciitilde}), and wrapping (\op{>\textasciitilde>}
and \op{<\textasciitilde<}), which determine the behaviour of the bits at either end.
In standard mode, bits shifted out are discarded and bits shifted in are set to 0
with the resulting value being of the same length as the original.
In extending mode, bits are not shifted out, instead the value is extended from the
other end with zeroes. In wrapping mode, bits shifted out are put back in on the
other side, with the resulting value being of the same length as the original.
...
\begin{prooftree}
    ...
    \BinaryInfC{$\Gamma\vdash A\ \op{<\textasciitilde}\ B : \ty{Bits}$}
\end{prooftree}

\begin{prooftree}
    \AxiomC{$\Gamma\vdash A : \ty{Bits}$}
    \AxiomC{$\Gamma\vdash B : \ty{Integer}$}
...
\begin{prooftree}
    \AxiomC{$\Gamma\vdash A : \ty{Bits}$}
    \AxiomC{$\Gamma\vdash B : \ty{Integer}$}
    \LeftLabel{Left Shift Extend}
    \BinaryInfC{$\Gamma\vdash A\ \op{<\textasciitilde\textasciitilde}\ B : \ty{Bits}$}
\end{prooftree}

\begin{prooftree}
    \AxiomC{$\Gamma\vdash A : \ty{Bits}$}
    \AxiomC{$\Gamma\vdash B : \ty{Integer}$}
    \LeftLabel{Right Shift Extend}
    \BinaryInfC{$\Gamma\vdash A\ \op{\textasciitilde\textasciitilde>}\ B : \ty{Bits}$}
\end{prooftree}
