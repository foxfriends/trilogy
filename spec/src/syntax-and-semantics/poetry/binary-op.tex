...
will not be an error, but actually the complex number \val{1i}.
...
Again, if the types are incorrect, it causes a runtime type error.

The three main binary bitwise operators extend the shorter operand with 0s on
the right until both are of the same length.
...
other end with zeros. Meanwhile, if the number provided to any of the shift
operators is negative, it is treated as a shift in the opposite direction of the
arrow.
