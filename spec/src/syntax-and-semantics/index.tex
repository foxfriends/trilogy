...
Slots are the most basic type of definition, consisting of a name and an expression
to bind as the value for that name. The expression is evaluated once, at the time the
module in which the slot is declared is instantiated.

Slots may be declared as mutable, allowing the value in the slot to be reassigned later.

\begin{bnf*}
    \bnfprod{SlotDefinition}{
        \bnfts{\kw{slot}}
        \bnfsp
        \bnfpn{Identifier}
        \bnfsp
...
Though there is no restriction on what value can be stored in a slot, the initial value
expression must be a valid ``constant expression''. Constant expressions may not involve
...
available at compilation or link time from some externally provided native library.
The string following the \kw{extern} keyword names the calling convention by which the
externally defined function is to be called. Valid options are \texttt{"c"} or \texttt{"trilogy"}.

At this time, it is likely incorrect to use the \kw{extern} keyword in any user program,
but it is frequently used within the core library.
