...
The \kw{then} or block handler neither cancels nor resumes automatically, but allows
the programmer to fully describe exactly the control flow that is desired. When
evaluation reaches the end of the block of such a handler, execution simply ends,
the \kw{cancel} or \kw{become} must be written explicitly.
...
ends instead. \kw{cancel} itself is also a first class keyword that can be captured
