...
The \kw{invert} handler simply inverts control flow, not cancelling or resuming, expecting
that the programmer will use imperative constructs to manually craft the exactly control
flow as desired. When evaluation reaches the end of the block of the \kw{invert} handler,
execution simply fizzles.

In each case, the first-class keywords \kw{resume} and \kw{cancel} are bound to the
...
the end without \kw{cancel}, execution fizzles instead. Of course, \kw{cancel} itself
...
As you can see, the \kw{invert} form is the most controlled, while the \kw{yield}, \kw{resume}
