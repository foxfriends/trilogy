...
When a pattern in a default binding context fails to match, we say the execution
...
failure, and will exit with an error status code. This error status
code may also be accompanied by a printed error message describing
to the user the circumstances of the program's failure, for debugging
purposes.

If the program completes via multiple executions it is again considered
a failure, even if all those executions were successful. This is because
a program running in the real world is restricted to determinism and
may only exit with a single value; a program exiting by more than one
path in this way is not something supported by any operating system.
