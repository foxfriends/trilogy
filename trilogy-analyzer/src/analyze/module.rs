...
use trilogy_lexical_ir::{Export, Id, ItemKey, Module};
use trilogy_parser::syntax::{Alias, DefinitionItem, Document};
use trilogy_parser::Spanned;

pub(crate) fn analyze_module(analyzer: &mut Analyzer, ast: Document) -> Module {
    let span = ast.span();
    ...
    let _top_level_ids = extract_top_level_ids(analyzer, &ast);
