...
static trilogy_callable_value* trilogy_callable_value_init(
    ...
    callable->end_to = NULL;
    if (end_to != NULL) {
        callable->end_to = trilogy_callable_assume(end_to);
        *end_to = trilogy_undefined;
    }
...
void trilogy_callable_end_to_into(
    trilogy_value* val, trilogy_callable_value* cal
) {
    if (cal->end_to == NULL) return;
    trilogy_callable_clone_into(val, cal->end_to);
}
